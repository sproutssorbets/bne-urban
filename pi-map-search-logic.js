...
function syncRefineField(){
  ...
  /* The bottom bar doesn't exist on screen at all until there's something to
     search for — an address or at least one filter term. Showing an empty,
     unusable bar next to the "Find an address" field was the confusing
     part; removing it entirely (not just disabling it) is more direct than
     dimming it. The advanced-search-btn (always visible) is the way in for
     anyone who wants the filter list without touching the map first. */
  const hasSelection = prettyParts.length > 0;
  const refineTop = document.getElementById('refine-top');
  const refineSpacer = document.getElementById('refine-spacer');
  if(refineTop) refineTop.classList.toggle('is-visible', hasSelection);
  if(refineSpacer) refineSpacer.classList.toggle('is-visible', hasSelection);
}
...
async function pickAddress(idx){
  ...
  /* Picking a suggestion only previews it on the map — it does not commit
     to the search yet. Committing (setting currentAddressTerm, updating the
     chip list, jumping to the filter section) happens only when the visitor
     explicitly clicks "Add to search" in the popup below, the same
     click-to-commit pattern the documented-pin popups already use. This
     gives them a chance to check the map first before deciding it's the
     right place. */
  ...
  const buildPopup = () => {
    ...
    return new mapboxgl.Popup({closeButton:true, maxWidth:'240px', offset:16})
      ...
      .setHTML(`<div style="padding:10px 12px 12px;font-family:Inter,sans-serif;">
        ...
        ${nearbyHtml}
        <button type="button" class="pi-popup-add" style="margin-top:10px" onclick="confirmAddressSelection('${escapedTitle}')">Add to search →</button>
      </div>`);
  };

  /* Opens on both mobile and desktop now — it carries the only way to
     actually commit the address, so it's no longer redundant screen space
     on mobile the way the old, button-less version was. */
...
/* Called from the search-result popup's "Add to search" button — this is
   the actual commit step pickAddress() used to do immediately on its own;
   now it only runs once the visitor confirms, not the moment they pick a
   suggestion from the dropdown. */
function confirmAddressSelection(title){
...
function initMap(){
 ...
 /* 2. Inicjujemy mapę używając zapisanych (lub domyślnych) wartości */
  ...
  /* Two fingers previously did what Mapbox does by default: pan AND zoom
     AND rotate AND tilt, all from the same gesture. That's why two fingers
     appeared to change the horizon angle — that was pitch, not a bug, just
     unwanted default behaviour. Keep two-finger pan and pinch-zoom, drop
     rotation and pitch entirely, so two fingers only ever move the map
     up/down/left/right and zoom it, nothing else. */
  ...
  map.on('load', () => {
    ...
    /* Desaturate all base map layers, keep data layers coloured.
       ...
       blue channel a few points below red/green — carried over from
       matching the site's cream --bg rather than being true neutral gray.
       These are R=G=B, so no hue survives at all, only lightness. */
    const baseLayers = map.getStyle().layers.map(l => l.id);
    baseLayers.forEach(id => {
      const layer = map.getStyle().layers.find(l => l.id === id);
      if(!layer) return;
      try {
        if(layer.type === 'background') map.setPaintProperty(id, 'background-color', '#F0F0F0');
        if(layer.type === 'fill') { map.setPaintProperty(id, 'fill-color', '#E5E5E5'); map.setPaintProperty(id, 'fill-outline-color', '#D0D0D0'); }
        if(layer.type === 'line') map.setPaintProperty(id, 'line-color', '#C8C8C8');
...
function renderDataLayers(map){
    ...
    /* The tooltip is positioned once, at tap/hover time, in screen pixels.
       ...
       Bound two ways on purpose: Mapbox's own 'movestart' (covers drag,
       pinch-zoom, scroll-zoom, programmatic flyTo), plus a raw touchmove
       listener straight on the map container as a belt-and-suspenders
       fallback, in case a particular mobile browser is slow to fire
       'movestart' for a finger-drag. */
    map.on('movestart', hideTooltip);
    map.getContainer().addEventListener('touchmove', hideTooltip, {passive:true});
...
───────────────────────────────────────── */
/* ─────────────────────────────────────────
   REFINE-SECTION FULL-BLEED FIX
───────────────────────────────────────── */
/* #refine-section needs to reach both screen edges on mobile once the map
   becomes full-screen, but the CSS-only breakout trick (width:100vw plus a
   negative margin assuming the parent Container is centered on the
   viewport) produced an offset instead, because that assumption didn't
   hold for wherever this Embed actually sits in the page. Measuring the
   real gap with getBoundingClientRect() and cancelling out exactly that
   many pixels works regardless of how the parent is positioned — no
   assumption to be wrong about. window.innerWidth is used instead of the
   100vw CSS unit for the same reason: some browsers count the scrollbar
   in 100vw, causing a few pixels of horizontal overflow; innerWidth never
   does. */
function fixRefineSectionWidth(){
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Compact icon-only buttons on narrow screens, so inputs can stay
     full-width. querySelectorAll, not querySelector — there are now two
     .btn-search-archive buttons (top "Advanced search" and bottom "Search
     archive"), and a single querySelector would only ever catch the first
     one in DOM order, leaving the other full-length. */
  if (window.matchMedia('(max-width: 900px)').matches) {
    const ri = document.getElementById('refineInput');
    if (ri) ri.placeholder = 'Pick address or category';
    document.querySelectorAll('.btn-search-archive').forEach(btn => { btn.textContent = '→'; });
  }

  const smartInput = document.getElementById('smartInput');
  smartInput.addEventListener('input', onSmartInput);

  /* refineInput is readonly and never accepts typing — a click there is a
     visitor trying to type a search directly, unaware it only fills in
     from picks elsewhere. Show a brief tooltip explaining that instead of
     silently doing nothing. */
  let refineLockedTimer;
  const refineInput = document.getElementById('refineInput');
  if(refineInput){
    refineInput.addEventListener('click', () => {
      const hint = document.getElementById('refine-locked-hint');
      if(!hint) return;
      hint.classList.add('vis');
      clearTimeout(refineLockedTimer);
      refineLockedTimer = setTimeout(() => hint.classList.remove('vis'), 1800);
    });
  }
  ...
  fixRefineSectionWidth();
  let refineWidthTimer;
  window.addEventListener('resize', () => {
    clearTimeout(refineWidthTimer);
