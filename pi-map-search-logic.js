...
function syncRefineField(){
  ...
  /* Bottom bar stays visually and functionally inactive until an address or
     a filter term is picked — it never accepts typed input either way
     (readonly), but looking equally "live" as the top search field was the
     confusing part. Disabling it removes that ambiguity about which field
     to use first. */
  const hasSelection = prettyParts.length > 0;
  const searchBtn = document.querySelector('.btn-search-archive');
  const wrap = document.getElementById('refineBarWrap');
  if(searchBtn) searchBtn.disabled = !hasSelection;
  if(wrap) wrap.classList.toggle('is-empty', !hasSelection);
}
