...
function addLocationToSearch(addressText){
  ...
  /* Previously auto-scrolled straight to #refine-section, skipping the map
     entirely and leaving no way back to it. Now just hint — the visitor
     stays on the map and only moves to the filters if they choose to. */
  showFilterPrompt();
}
...
/* Same #toast element as showToast, but clickable and shown for longer
   (5s vs 2s) since this one needs a tap, not just a glance. Reusing the
   element rather than adding a second one keeps only one such message on
   screen at a time. */
function showFilterPrompt(){
  const t = document.getElementById('toast');
  t.textContent = 'Refine further ↓';
  t.classList.add('is-action', 'vis');
  t.onclick = () => {
    document.getElementById('refine-section').scrollIntoView({behavior:'smooth', block:'start'});
    t.classList.remove('vis', 'is-action');
    t.onclick = null;
  };
  clearTimeout(toastTimer);
