...
(function () {
  ...
  function mount() {
    ...
    /* Sidebar layout: panel and grid sit side by side inside a shared
       flex wrapper, not stacked as two independent siblings. Insert
       the wrapper where the grid used to be, then move the grid into
       it alongside the panel — the grid element itself, its listeners
       and PhotoDeck's own masonry state are untouched, only its
       position in the tree changes. */
    var wrap = document.createElement('div');
    wrap.id = 'pi-facets-wrap';
    grid.parentNode.insertBefore(wrap, grid);

    var gridCol = document.createElement('div');
    gridCol.className = 'pi-facets-grid-col';
    ...
    /* The grid is narrower now that the sidebar sits beside it.
       PhotoDeck's masonry computed its original layout against the
       old, wider container, so it needs the same relayout nudge as
       filtering does. */
    if (window.dispatchEvent) {
